    ...
    W: StrWrite,
{
    ...
    fn start_tag(&mut self, tag: Tag<'a>) -> Result<(), W::Error> {
        match tag {
            ...
            Tag::BlockQuote(kind) => {
                let (class_str, title) = match kind {
                    None => ("", ""),
                    Some(kind) => match kind {
                        BlockQuoteKind::Note => (" class=\"markdown-alert-note\"", "Note"),
                        BlockQuoteKind::Tip => (" class=\"markdown-alert-tip\"", "Tip"),
                        BlockQuoteKind::Important => {
                            (" class=\"markdown-alert-important\"", "Important")
                        }
                        BlockQuoteKind::Warning => (" class=\"markdown-alert-warning\"", "Warning"),
                        BlockQuoteKind::Caution => (" class=\"markdown-alert-caution\"", "Caution"),
                    },
                };
                if self.end_newline && !class_str.is_empty() {
                    self.write(&format!(
